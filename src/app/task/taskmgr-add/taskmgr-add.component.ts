...
export class TaskmgrAddComponent implements OnInit {
  ...
  public parentTaskDet: ParentTaskDetail[];
  public users: User[];
  public parentTaskDetails: TaskDetail[];
  public taskDetail: TaskDetail;
  public parentTask: ParentTaskDetail;
  results: string
  ...
  constructor(private projectService: ProjectService, private userService: UserService,private parentTaskService:ParentTaskService,
    private taskManagerService: TaskService, private router: Router) { }

  ngOnInit() {
    this.taskDetail = new TaskDetail();
    this.parentTask = new ParentTaskDetail();
  ...
  onAddTask() {
    ...
    if(this.isParentTaskSelected){
      this.parentTask.parentTask=this.taskDetail.name;
      this.parentTaskService.AddTask(this.parentTask).subscribe(response => {
  ...
  onGetAllParentTask() {
    this.parentTaskService.GetParentList().subscribe(
      response => this.parentTaskDet = response) ;
  }
}
