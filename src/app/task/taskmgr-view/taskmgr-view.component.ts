...
export class TaskmgrViewComponent implements OnInit {
  ...
  public parentTasks:ParentTaskDetail[]; 
  ...
  constructor(private service: TaskService, private projectService: ProjectService,private parentService:ParentTaskService,
...
onGetAllParentTask(){
this.parentService.GetAllTasks().subscribe(
  p=>this.parentTasks=p);
}
  onGetAllProjects() {
  ...
  onSelectProject(selectedProject: Project) {
    ...
    this.service.GetAllTasks().subscribe(response => {
      (response as TaskDetail[]).filter(resElement => resElement.projectId == this.project.projectId).
        forEach(element => {
          let taskDetail = this.parentTasks.find(res => res.parentId == element.parentId);
          if (taskDetail != undefined)
            element.parentName = taskDetail.parentTask;
          else
